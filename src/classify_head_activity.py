...
def calculate_raw_attention_accuracy(attn_for_head, all_chunk_ids_for_batch, args):
    """
    ...
    for i in range(1, n_chunks):
        ...
        score = row_ideal[batch_indices, most_attn_idx.to(row_ideal.device)]
        head_accs[:, i] = score
    ...
    if n_chunks > 10:
        ...
        learning_score = head_accs.mean(dim=0)[1:].mean()
    else:
...
def main():
    ...
    head_activities = defaultdict(list)
    ...
    if args.module != 'heads':
        ...
        for layer in range(config.num_hidden_layers):
            ...
            for head in range(n_heads):
                ...
                raw_acc, head_acc_matrix = calculate_raw_attention_accuracy(attn_for_head, all_chunk_ids, args)
                head_accuracies.append((raw_acc, head))
    ...
    for target in targets:
        if args.module == 'heads':
            ...
            _, head_acc_matrix = calculate_raw_attention_accuracy(attn_for_head, all_chunk_ids, args)
        ...
        head_activities[address].append(activity_for_classification.float().cpu().numpy())
        head_labels[address].append(labels.float().cpu().numpy())

    # --- Concatenate data from all batches ---
    for address in head_activities:
        head_activities[address] = np.concatenate(head_activities[address], axis=0)
        head_labels[address] = np.concatenate(head_labels[address], axis=0)
    ...
    for target in targets:
        ...
        results[address] = acc
        print(f"Module {args.module} {address}: Classification accuracy = {acc:.4f}")

    save_dir = Path('data/classification_results')
    save_dir.mkdir(parents=True, exist_ok=True)
    model_str = args.model_name.split("/")[-1]
    torch.save(results, f'{save_dir}/{model_str}_{args.module}_classification_accs.pt')
