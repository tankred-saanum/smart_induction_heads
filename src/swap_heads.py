...
induction_score_arr = torch.load(f'data/induction_scores/{args.model_name.split("/")[-1]}.pt')
best_head_address, worst_head_address = get_best_and_worst(score_arr, induction_scores=induction_score_arr, threshold=0.4)
best_head_address[0]
worst_head_address
accuracies = []
best_head_inputs =[]
...
with torch.no_grad():
    with model.trace(batched_tokens, output_attentions=False):
        # get the input to the attention layer
        best_input = model.model.layers[best_head_address[0]].input.save()
        #best_head_inputs.append(best_input)           
...
with torch.no_grad():
    with model.trace(batched_tokens, output_attentions=True):
        # get the input to the attention layer
        #best_input = model.model.layers[best_head_address[0]].input.save()
        true_input = model.model.layers[worst_head_address[0]].self_attn.k_proj.input
        norm = torch.linalg.vector_norm(true_input, dim=-1, keepdim=True)
        
        norm_best_input = best_input/torch.linalg.vector_norm(best_input, dim=-1, keepdim=True)
        norm_best_input = norm_best_input*norm
        model.model.layers[worst_head_address[0]].self_attn.k_proj.input=norm_best_input
        model.model.layers[worst_head_address[0]].self_attn.q_proj.input=norm_best_input
...
accuracies.mean(dim=0)
save_dir = Path(f'data/swapped_learning_scores/{args.ablation_style}_threshold={args.threshold}/{order}/{args.model_name.split("/")[-1]}')
...
#     heads = layer_dict[layer]
#     for head in heads:
layer = worst_head_address[0]
head = worst_head_address[1]
address = f'{layer}-{head}'
