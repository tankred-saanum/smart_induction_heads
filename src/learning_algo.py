...
batched_tokens.shape
latent_size=64
margin = 1.0
margin = torch.tensor([margin]).to(device)
x = torch.arange(batched_tokens.size(1)).to(device)
embedder = torch.nn.Linear(batched_tokens.size(1), latent_size).to(device)
optimizer = torch.optim.Adam(embedder.parameters(), lr=0.001)
chunked_tokens = batched_tokens.view(args.n_reps*args.n_permute, args.chunk_size)
comparison = chunked_tokens[1:] == chunked_tokens[:-1]
comparison = comparison.sum(dim=-1)
comparison = (comparison == args.chunk_size).unsqueeze(-1).repeat(1, args.chunk_size).float()
n_iters=250
for iter in range(n_iters):
    z = embedder(torch.nn.functional.one_hot(x, batched_tokens.size(1)).float())
    z_chunked = z.view(args.n_reps*args.n_permute, args.chunk_size, latent_size)
    z_chunked_pooled = z_chunked.mean(dim=1)
    dists = (z_chunked_pooled[1:] - z_chunked_pooled[:-1]).pow(2).sum(dim=-1)
    pos_loss = dists*comparison.max(dim=-1)[0]
    neg_loss = torch.maximum(-margin, dists *  (comparison.max(dim=-1)[0]-1))
...
z=z_chunked_pooled.detach().cpu()
plt.scatter(z[15:, 0], z[15:, 1])
plt.show()
