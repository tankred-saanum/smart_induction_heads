    ...
    if args.markov_order==2:
        ...
        for i, l in enumerate(ldict.keys()):
            for h in ldict[l]:
                ...
                ax[headmap_idx, 0].imshow(attn, cmap='copper')
                ...
                ax[headmap_idx, 0].set_xticks(torch.arange(len(seq_letters)))
                ax[headmap_idx, 0].set_yticks(torch.arange(len(seq_letters)))
                ax[headmap_idx, 0].set_xticklabels([f"${letter}$" for letter in seq_letters], fontsize=fs-5)
                ax[headmap_idx, 0].set_yticklabels([f"${letter}$" for letter in seq_letters], fontsize=fs-5, rotation=0)
                reps = torch.arange(args.n_reps*args.n_permute)
                ax[headmap_idx, 1].plot(reps, head_accs.mean(dim=0)*100, color=colors[0], linestyle=style)
                ax[headmap_idx,1].plot(torch.arange(args.chunk_size*args.n_reps*args.n_permute)/args.chunk_size, induction_accs.mean(dim=0)*100, color=colors[1], linestyle=style)
                ax[headmap_idx,1].set_ylabel('Accuracy %')
                ax[headmap_idx,1].set_ylim([0, 101])
                if headmap_idx == 1:
                    ax[headmap_idx,1].set_xlabel('Repetition')
                    ax[headmap_idx,0].set_ylabel('Context match lesion')
                else:
                    ax[headmap_idx,0].set_ylabel('No lesion')
                    
                for j in range(grid_interval, attn.shape[1], grid_interval):
                    ax[headmap_idx, 0].axvline(x=j-0.5, color='white', linewidth=lwd)

                for j in range(grid_interval, attn.shape[0], grid_interval):
                    ax[headmap_idx, 0].axhline(y=j-0.5, color='white', linewidth=lwd)
