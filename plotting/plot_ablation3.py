...
for i, order in enumerate(markov_orders):
    ...
    accs = torch.load(f'data/ablated_learning_scores/random_threshold={args.threshold}/{order}/{args.model_name.split("/")[-1]}/model_accs.pt', weights_only=False)
    accs = torch.cat([accs, torch.ones(accs.size(0), 1)], dim=-1)
    ...
    accs = torch.load(f'data/ablated_learning_scores/one_back_threshold={args.threshold}/{order}/{args.model_name.split("/")[-1]}/model_accs.pt', weights_only=False)
    accs = torch.cat([accs, torch.ones(accs.size(0), 1)], dim=-1)
    ...
    if args.markov_order==2:
        ...
        accs = accs[:, mask]
        accs = accs.mean(dim=1)
    ax[i, 0].plot(accs*100, label='Lesioned', color=colors[2])
...
for i, order in enumerate(markov_orders):
    ...
    ablation='random'
    files = os.listdir(f'data/ablated_learning_scores/{ablation}_threshold={args.threshold}/{order}/{args.model_name.split("/")[-1]}')
    aggregate = []
    for _, file in enumerate(files):
        if file in exceptions:
            continue
        ...
        if induction_score > 0.4:
            ...
            accs = torch.load(f'data/ablated_learning_scores/{ablation}_threshold={args.threshold}/{order}/{args.model_name.split("/")[-1]}/{file}', weights_only=False)
            accs = accs.mean(dim=0)
            aggregate.append(accs)
    accs = torch.stack(aggregate, dim=0)
    if args.aggregate == 'mean':
    ...
    if args.aggregate == 'mean':
        ...
        _, max_idx = accs.mean(dim=-1).topk(5)
        accs = accs[max_idx].mean(dim=0)
    ax[i, 1].plot(accs*100, label='Lesioned', color=colors[2])
...
for i, order in enumerate(markov_orders):
    ...
    if args.aggregate == 'mean':
        ...
        _, max_idx = accs.mean(dim=-1).topk(5)
        accs = accs[max_idx].mean(dim=0)
    ax[i, 2].plot(accs*100, label=f'Random heads lesioned', color=colors[1])
    ...
    if args.aggregate == 'mean':
        ...
        _, max_idx = accs.mean(dim=-1).topk(5)
        accs = accs[max_idx].mean(dim=0)
    ax[i, 2].plot(accs*100, label=f'One-back heads lesioned', color=colors[2])
...
fig.suptitle(f'{args.model_name.split("/")[-1]}')
