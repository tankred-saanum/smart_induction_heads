...
for i, order in enumerate(markov_orders):
    ...
    for model_name in models:
        files = os.listdir(f'data/learning_scores_alt/{order}/{model_name.split("/")[-1]}')
        exp_args = torch.load(f'data/learning_scores_alt/{order}/{model_name.split("/")[-1]}/args.pt', weights_only=False)
        accs = torch.load(f'data/learning_scores_alt/{order}/{model_name.split("/")[-1]}/model_accs.pt', weights_only=False)
    ...
    for model_name in models:
        ...
        for _, file in enumerate(files):
            ...
            if induction_score > 0.4:
                ...
                accs = torch.load(f'data/learning_scores_alt/{order}/{model_name.split("/")[-1]}/{file}', weights_only=False)
                accs = accs.mean(dim=0)
...
for i, order in enumerate(markov_orders):
    ...
    for model_name in models:
        model_str= model_name.split("/")[-1]
        learning_scores = torch.load(f'data/learning_scores_alt/{order}/{model_str}/learning_scores.pt')
        induction_scores = torch.load(f'data/induction_scores/{model_name.split("/")[-1]}.pt')
        files = os.listdir(f'data/learning_scores_alt/{order}/{model_name.split("/")[-1]}')
        aggregate = []
        for _, file in enumerate(files):
...
#     accs = torch.load(f'data/learning_scores_alt/{order}/{args.model_name.split("/")[-1]}/{file}', weights_only=False)
#     accs = accs.mean(dim=0)
...
# accs = torch.load(f'data/learning_scores_alt/{order}/{args.model_name.split("/")[-1]}/model_accs.pt', weights_only=False)
# accs = torch.cat([accs, torch.ones(accs.size(0), 1)], dim=-1)
