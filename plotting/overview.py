...
axs[1,1].set_xticks(torch.arange(len(chunk_ids))*args.n_permute + (0.5*args.n_permute))
axs[1,1].set_yticks(torch.arange(len(chunk_ids))*args.n_permute + (0.5*args.n_permute))
...
add_copying_attention_borders(axs[2,0], seq_3_chunks.cpu().float(), torch.tensor(chunk_ids_3))
axs[2,0].set_xticks(torch.arange(len(chunk_ids_3)) + 0.5)
axs[2,0].set_yticks(torch.arange(len(chunk_ids_3)) + 0.5)
axs[2,0].set_xticklabels([f"${id_to_letter[id]}$" for id in chunk_ids_3], fontsize=large_fs)
axs[2,0].set_yticklabels([f"${id_to_letter[id]}$" for id in chunk_ids_3], fontsize=large_fs, rotation=0)
...
axs[2,1].set_yticklabels([f"${id_to_letter[id]}$" for id in chunk_ids_3], fontsize=large_fs, rotation=0)
...
axs[2,2].set_xticks(torch.arange(len(chunk_ids_3))*third_order_args.n_permute*args.n_permute_primitive + (0.5*third_order_args.n_permute*args.n_permute_primitive))
axs[2,2].set_yticks(torch.arange(len(chunk_ids_3))*third_order_args.n_permute*args.n_permute_primitive + (0.5*third_order_args.n_permute*args.n_permute_primitive))
axs[2,2].set_xticklabels([f"${id_to_letter[id]}$" for id in chunk_ids_3], fontsize=large_fs, rotation=0)
axs[2,2].set_yticklabels([f"${id_to_letter[id]}$" for id in chunk_ids_3], fontsize=large_fs, rotation=0)
