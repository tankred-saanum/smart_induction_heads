...
for i, order in enumerate(markov_orders):
    ...
    for model_name in models:
        ...
        if args.markov_order==2:
            ...
            normalizer = accs.size(0)**0.5
            accs_mean = accs.mean(dim=0)
            accs_std = accs.std(dim=0) / (normalizer)  # standard error
            accs_mean = accs_mean[:, :-1].mean(dim=1)
            accs_std = accs_std[:, :-1].mean(dim=1)
            ...
            mask[-1] = False
            accs_mean = accs_mean[:, mask]
            accs_std = accs_std[:, mask]
            accs_mean = accs_mean.mean(dim=1)
            accs_std = accs_std.mean(dim=1)
        ax[i, 0].errorbar(range(len(accs_mean)), accs_mean*100, yerr=accs_std*100, label=model_name)
...
for i, order in enumerate(markov_orders):
    ...
    for model_name in models:
        ...
        if args.aggregate == 'mean':
            ...
            accs_mean = accs[max_idx]
            accs_std = torch.zeros_like(accs_mean)  # no error for single selection
        elif args.aggregate == 'topk':
            _, max_idx = accs.mean(dim=-1).topk(5)
            accs_topk = accs[max_idx]
            accs_mean = accs_topk.mean(dim=0)
            accs_std = accs_topk.std(dim=0) / (normalizer)  # standard error
        ax[i, 1].errorbar(range(len(accs_mean)), accs_mean*100, yerr=accs_std*100, label=model_str)
...
for i, order in enumerate(markov_orders):
    ...
    for model_name in models:
        ...
        if args.aggregate == 'mean':
            ...
            accs_mean = accs_topk.mean(dim=0)
            accs_std = accs_topk.std(dim=0) / (normalizer)  # standard error
        ax[i, 2].errorbar(range(len(accs_mean)), accs_mean*100, yerr=accs_std*100, label=model_str)
#ax[1, 0].set_ylabel('Accuracy %')
